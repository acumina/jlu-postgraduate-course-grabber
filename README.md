...
| 2 | 打开选课网站 `https://yjsxk.jlu.edu.cn/yjsxkapp/sys/xsxkapp/index.html` → 面板 → 设置 → **配置导入/导出** → 点 **「载入内置预设」** | 一键配好生效站点、提交/查询模板、判定规则、运行页面限制。**这一步不能省**：默认白名单是空的，不载入预设面板就不会在这个网站上出现 |
| 3 | 在网站上**正常登录**（含验证码）→ 点进**选课页面**（`course.html`） | 登录无法自动化（图形验证码），这是设计上的边界。**进选课页引擎就会自动开始跑**，不用点启动 |
| 4 | （只在需要时）面板 → **余量/档案** 页 → 搜课程名 → 勾选 → 「把勾选的加入监控」 | 加完目标就会自动抢。**引擎默认已处于运行状态**，不需要点「启动」；只有你手动停过之后才需要再点一次 |

**载入预设之后自动进行的事**（你不用做）：

- **面板只在选课网站上出现**（预设里带了生效站点 `yjsxk.jlu.edu.cn`）
