...
| 插件生效范围 | **只在 `yjsxk.jlu.edu.cn` 上出现面板**（预设里带的生效站点），其它网站完全不动 |
| 接口 | 提交 `choiceCourse.do`、查询余量 `loadGxkCourseInfo.do`、已选课程 `loadStdCourseInfo.do` |

换别的学校/系统也能用，但站点白名单、提交/查询模板、判定文案都得自己配一遍
（抓一次真实请求就能自动学出来，见 [用 HAR 自动生成配置](#用-har-自动生成配置)）。
...
| 场景 | 会发生什么 |
| --- | --- |
| **首次装上扩展**（从没点过启动） | 默认就是"运行中"。**一进入选课页（`course.html`）引擎自动开始轮询/抢课**，有目标就会提交 |
| 抢课中途刷新页面 / 重开浏览器 | **自动恢复继续抢**（上次是抢课状态） |
| 掉线 → 你重新登录（含验证码） | **自动继续**，不用手动点启动 |
| **你手动点过「停」**（或撞上验证码自动停机） | **不会**自己恢复 —— 这是刻意的：主动停机后必须你自己再点一次「启动」 |
| 打开的是首页 `index.html` 而不是选课页 | 只有面板，引擎不跑（引擎必须在选课页）；点进选课页就会接着跑 |

> 为什么"手动停过就不自动恢复"：这是安全边界。撞上验证码、或你临时想收手时，
> 插件不能背着你继续发请求 —— 所以**主动停机是粘性的**，只有明确的登录态变化才会自动继续。

## 全新环境快速开始（2 步手动，其余全自动）

> **不想用 git？** 到 [Releases](../../releases/latest) 下载 ZIP → 解压 → `chrome://extensions`
> 打开「开发者模式」→「加载已解压的扩展程序」选那个文件夹即可（ZIP 里 `manifest.json` 就在根目录）。

不需要 Node、不需要构建、不需要装任何依赖，**也不需要手动导入配置**
（项目里的 `kx-config-吉大研究生选课.json` 会在启动时**自动应用**）：

| # | 你要做的 | 说明 |
| --- | --- | --- |
| 1 | `chrome://extensions` → 打开**开发者模式** → **加载已解压的扩展程序** → 选本目录（或解压出来的文件夹） | 一次性 |
| 2 | 打开 `https://yjsxk.jlu.edu.cn/yjsxkapp/sys/xsxkapp/index.html`，**正常登录**（含验证码）→ 点进**选课页面**（`course.html`） | 登录无法自动化（图形验证码），这是设计上的边界。**进选课页引擎就会自动开始跑**，不用点启动、不用点载入预设 |

之后加目标也是可选的：到 **档案/余量** 页搜课程名 → 勾选 → 「把勾选的加入监控」，
加完立刻自动抢（引擎默认已处于运行状态；只有你手动停过之后才需要再点「启动」）。

## 明年 / 换电脑：零手动流程

教学班 ID 每年都变（形如 `20261-101-A0162101001-1785413134156`），
所以**跨年能带走的只有课程名**。插件用一个「**备选清单**」承载它：

### 今年（一次，2 分钟）

1. 档案页搜到想选的课 → 勾选 → 点 **「加入备选清单（明年自动找回）」**
   （只记 课程名/教师/校区，不记教学班 ID）
2. 点 **「复制为配置片段（贴进项目配置文件）」** → 把复制到的内容替换
   `kx-config-吉大研究生选课.json` 里的 `"wishlist"` 字段 → 保存（愿意的话提交到你的仓库）

### 明年 / 新电脑（**只有登录是手动**）

```
装扩展（解压 ZIP → 加载已解压的扩展程序）
   ↓ 自动：启动时读取项目配置 → 备选清单变成「按名字监控」的目标
打开首页登录（输验证码，这是唯一的手动步骤）
   ↓ 自动：点进选课页 → 拉一次全量课表
...
**中间不需要你点任何按钮** ✅ —— 目标里那些「还没有教学班ID」的条目是**正常状态**，
面板会用绿框明确写清楚"这是等选课页拉课表后自动匹配"。

想关掉兜底（只在相似度足够高时才自动采用，存疑留给你手工选）：
设置里把 `engine.autoResolveFallback` 设为 `false`。

**自动进行的事**（你不用做）：

- **面板只在选课网站上出现**（项目配置里带着生效站点 `yjsxk.jlu.edu.cn`，启动时**自动应用**）
- **进入选课页自动开始**（首次安装默认就是运行中；不需要点「启动」）
