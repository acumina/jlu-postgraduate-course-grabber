...
(function () {
  ...
  function sessionTick() {
    ...
    if (left !== null) {
       ...
       *      autoOpenedLogin 重置，于是一遍遍开新页（背景："登录时间快结束了疯狂弹出新页面"）
  ...
  async function start(reason, opts) {
     ...
     * 这些目标永远拿不到 ID（实测反馈"进选课页不自动选课"的原因之一）。 */
  ...
  async function resolveTargetsByName(opts) {
    ...
    /* 最大兜底（设计要求："查找模糊课程或相似课程最大兜底"、"抢错了可以退课，比模糊不到更好"）：
    ...
    let fallbackCount = 0;
    let expandedCount = 0;
    const extras = [];        // 同名/相似的其它教学班 → 追加成新目标（设计要求"全部都要"）
   ...
   *  **不够也采用最像的那个（最大兜底）** —— 设计要求"抢错了可以退课，比模糊不到更好"。
   ...
   * 于是每次改了文件都得手动导入一次（背景："这个配置json能自动配置吗"）。
   *
  ...
  /**
   * 全新安装时**自动配好生效站点**（设计要求："明年使用是直接从零开始，跳过加入白名单的流程"）。
   *
   * 两道保险：
      ...
      if (globalThis.KXPanel) {
        try {
          chrome.storage.local.get(UI_KEY).then(function (got) {
             ...
             *   去年的课表挑成愿望单（设计要求："在登陆页面弹出去年的课表加入愿望单"）。
