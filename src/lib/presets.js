...
globalThis.KXPresets = {
  'jlu-yjsxk': {
    ...
    "engine": {
      ...
      "_resolveNote": "同名班策略（设计要求）：默认 **不自动收手**，同名教学班一直一起抢。理由：多抢一个只是去退一次课（可逆），而误停一个是丢掉课程（不可逆）；而\"是否抢到了\"的判定本身不完全可靠（曾把成功响应判成\"无法判定\"，白抢一小时）。模糊门槛相应放低：autoResolveMinScore=0.6、autoResolveMinGap=0.04。想省得退课，可把 autoStopSameName 设为 true —— 它只在「已选课程」列表（服务器权威）确认后才停同名的其它班。",
      ...
      "expandMax": 30,
      "_expandNote": "设计要求：最大兜底选课要选择全部模糊命中的课程，不能选一门抢就不抢其他的了。expandAllMatches=true：按课程名解析时，把够像的**全部**教学班都加入监控（羽毛球×12 就建 12 个目标，哪个先有名额就抢哪个）。expandMax=30：单个课程最多展开这么多教学班（防止某门课有几百个班把列表刷爆）。抢到之后**不会**自动停掉同名其它班（engine.autoStopSameName=false）——多抢到只是多退一次课，漏掉就整轮错过。",
      "_rateNote": "设计要求：默认轮询速率上限过低 —— 至少做到首次抢课延迟足够低，反复轮询在 500ms 间隔以内。现在：intervalMs=200（一轮基础间隔）、minGapMs=100（两批发包最小间隔，远小于 500ms）、maxReqPerMinute=600、maxConcurrent=12、jitterPct=15。注意：浏览器会把后台标签页的定时器压到每分钟 1 次 —— 抢课期间必须让选课页保持可见。"
    },
    "query": {
    ...
    "_debugNote": "发布默认：关闭自动推送（新用户没有本地收集器，开着只会一直失败）。你自己的收集器地址与开关存在浏览器里，应用配置时会保留（KX.mergeBundledConfig），不受这里影响。",
