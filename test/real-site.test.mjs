...
test('吉大: 宽松兜底的 success 规则 + 成功放在判定链最后（设计要求：宁可误判成功也不错失课程）', () => {
  /* 设计要求："选课成功最大兜底即可，选上了继续轮询也没问题"。
   * 于是两条设计一起用：
...
test('回归: 进入页面就自动开始 —— 只由 enabled 一条规则决定', () => {
  ...
  assert.ok(/\(raw === undefined \|\| raw === null\) \? true : !!raw/.test(src),
    '首次使用（键不存在）默认 true —— 设计要求"点入页面就自动进行"');
  ...
  /* 真实 bug（在新环境实测踩到）：halt() 里判断的是一个**根本不存在的变量**
...
test('回归: 自动打开登录页只认"真掉线"，且全局限流（真实事故：疯狂弹新页面）', () => {
  /* 背景："现在登陆时间快结束了也疯狂弹出新页面，改为只有扫到掉线了才弹新页面"。
   * 两个原因：
